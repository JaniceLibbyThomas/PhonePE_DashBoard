...
if(user_ip1 =='ALL INDIA'):
    ...
    if(trans_r_user =='TRANSACTIONS'):
      ...
      out= run_query(f"SELECT  `tr_Name`, `tr_Count`, `tr_Amount` FROM `MAP_TRANS_AGG_INDIA` WHERE tr_Year_val ='{Year}' and tr_Quater='{Quater}'")
      ...
      with tab2:
        #Top 10: State, District and Pincode
          ...
          if(radio == 'State'):
            out = run_query(f"SELECT `tr_stat_name`, `tr_count`, `tr_amt` FROM `TOP_TRANS_AGG_INDIA_State` WHERE tr_Year_val ='{Year}' and tr_Quater='{Quater}'")
           # out=mycursor.fetchall()
            ...
            out = run_query(f"SELECT `tr_pincode`, `tr_count`, `tr_amt` FROM `TOP_TRANS_AGG_INDIA_dist_pincode` WHERE tr_Year_val ='{Year}' and tr_Quater='{Quater}'")
            #out=mycursor.fetchall()
            ...
            out = run_query(f"SELECT `tr_dist_name`, `tr_count`, `tr_amt` FROM `TOP_TRANS_AGG_dist1` WHERE tr_Year_val ='{Year}' and tr_Quater='{Quater}'")
            #out=mycursor.fetchall()
      ...
      with tab3:
            ...
            out = run_query(f"SELECT `tr_State_name`, `tr_Count`, `tr_amount`, `tr_Map_id` FROM `HOVER_TRANS_AGG_INDIA`  WHERE tr_Year_val ='{Year}' and tr_Quater='{Quater}'")
            #out=mycursor.fetchall()
      ...
      #Total Online User details
              ...
              out = run_query(f"SELECT `us_No_of_Registered_User`, `us_AppOpens`, `us_Count` FROM `MAP_USER_AGG_INDIA` WHERE us_Year_val ='{Year}' and us_Quater='{Quater}'")
              #out=mycursor.fetchall()
        ...
        #Top 10: State, District and Pincode
              ...
              if(radio == 'State'):
                out = run_query(f"SELECT `tr_state_name`, `No_of_Registered_User` FROM `TOP_User_AGG_state` WHERE tr_Year_val ='{Year}' and tr_Quater='{Quater}'")
                #out=mycursor.fetchall()
                ...
                out = run_query(f"SELECT `tr_dist_name`, `No_of_Registered_User` FROM `TOP_User_AGG_dist` WHERE tr_Year_val ='{Year}' and tr_Quater='{Quater}'")
                #out=mycursor.fetchall()
                ...
                out = run_query(f"SELECT `tr_pincode`, `No_of_Registered_User` FROM `TOP_User_dist_pincode1` WHERE tr_Year_val ='{Year}' and tr_Quater='{Quater}'")
                #out=mycursor.fetchall()
          ...
          with tab3:
                ...
                india_states = json.load(open("/Users/joelsanthoshraja/Downloads/states_india.geojson", "r"))
                out = run_query(f"SELECT `us_state_name`, `us_reg_us`, `us_appOpen`, `us_Map_id` FROM `HOVER_USER_STATE`  WHERE us_Year_val ='{Year}' and us_Quater='{Quater}'")
    ...
    if(trans_r_user =='TRANSACTIONS'):
      ...
      #Total Online Transaction details
        ...
        out = run_query(f"SELECT  `tr_Name`,  `tr_Count`, `tr_Amount` FROM `MAP_TRANS_AGG_STATE` WHERE tr_Year_val ='{Year}' and tr_Quater='{Quater}' and tr_state ='{state}'")
        #out=mycursor.fetchall()
        Frame1 = pd.DataFrame(out,columns = ['Type_of_Transaction','Count','Amount'])
        for i in range(0,len(list(Frame1['Amount']))):
            t = (Frame1['Amount'][i]).split('.')
      ...
      with tab2:
        #Top 10: State, District and Pincode
          ...
          if(radio == 'District'):
            ...
            out = run_query(f"SELECT `tr_dist_name`, `tr_count`, `tr_amt` FROM `TOP_TRANS_AGG_dist1` WHERE tr_Year_val ='{Year}' and tr_Quater='{Quater}' and tr_state = '{state}'")
            #out=mycursor.fetchall()
            Frame2 = pd.DataFrame(out,columns = ['District Name','Count','Amount'])
            temp = Frame2.groupby(['District Name'])['Count'].sum()
            data = dict(temp)
            ...
            out = run_query(f"SELECT `tr_pincode`, `tr_count`, `tr_amt` FROM `TOP_TRANS_AGG_INDIA_dist_pincode1` WHERE tr_Year_val ='{Year}' and tr_Quater='{Quater}' and tr_state = '{state}'")
            #out=mycursor.fetchall()
            Frame2 = pd.DataFrame(out,columns = ['Pincode','Count','Amount'])
            temp = Frame2.groupby(['Pincode'])['Count'].sum()
            data = dict(temp)
      ...
      #Total Online User details
        ...
        #Top 10: State, District and Pincode
              ...
              if(radio == 'District'):
                out = run_query(f"SELECT `tr_dist_name`, `No_of_Registered_User` FROM `TOP_User_AGG_dist1` WHERE tr_Year_val ='{Year}' and tr_Quater='{Quater}' and tr_state = '{state}'")
               # out=mycursor.fetchall()
                Frame2 = pd.DataFrame(out,columns = ['District Name','No of Registered User'])
                ...
                out = run_query(f"SELECT `tr_pincode`, `No_of_Registered_User` FROM `TOP_User_dist_pincode1` WHERE tr_Year_val ='{Year}' and tr_Quater='{Quater}' and tr_state = '{state}'")
               # out=mycursor.fetchall()
                Frame2 = pd.DataFrame(out,columns = ['Pincode','No of Registered User'])
# Frame2['No Of Registered User'] = Frame2['No Of Registered User'].astype(int)
                temp = Frame2.groupby(['Pincode'])['No of Registered User'].sum()
